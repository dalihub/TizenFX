...
namespace Tizen.NUI
{
    ...
    internal class LayoutGroupEx : LayoutItemEx
    {
        ...
        protected override void OnLayout(bool changed, LayoutLengthEx left, LayoutLengthEx top, LayoutLengthEx right, LayoutLengthEx bottom)
        {
            ...
            for( uint i = 0; i < ChildCount; ++i )
            {
                ...
                if( childLayout )
                {
                    ...
                    var childLeft = new LayoutLengthEx( childOwner.Position2D.X );
                    var childTop = new LayoutLengthEx( childOwner.Position2D.Y );
        ...
        {
            View childOwner = child.GetOwner();
        ...
        protected virtual void MeasureChildWithMargins(LayoutItem child, MeasureSpecification parentWidthMeasureSpec, LayoutLengthEx widthUsed, LayoutMeasureSpec parentHeightMeasureSpec, LayoutLengthEx heightUsed)
        {
            View childOwner = child.GetOwner();
            int desiredWidth = childOwner.WidthSpecification;
            int desiredHeight = childOwner.HeightSpecification;
