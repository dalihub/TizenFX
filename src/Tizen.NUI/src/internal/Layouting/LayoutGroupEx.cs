...
namespace Tizen.NUI
{
    ...
    internal class LayoutGroupEx : LayoutItemEx, ILayoutParentEx
    {
        ...
        private void ChildAddedToOwner(View child)
        {
            Log.Info("NUI", "ChildAddedToOwner:" + child.Name + " to " + Owner.Name + "\n");
            ...
            if (oldParent != Owner)
            {
                ...
                if ((true == Owner.layoutSet || GetType() == typeof(View)) && null == child.LayoutEx && false == View.layoutingDisabled)
                {
        ...
        {
            ChildAddedToOwner(childAddedEvent.Added);
        }
