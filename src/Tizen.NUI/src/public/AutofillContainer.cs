...
namespace Tizen.NUI
{
    ...
    public class AutofillContainer : BaseHandle
    {
        ...
        private BaseComponents.TableView listView;
        uint listIndex = 0;
        ...
        private delegate bool AuthenticationEventCallbackType(IntPtr autofillContainer);
        private delegate void ListEventCallbackType(IntPtr control, string text);
        ...
        internal AutofillContainer(global::System.IntPtr cPtr, bool cMemoryOwn) : base(Interop.AutofillContainer.AutofillContainer_SWIGUpcast(cPtr), cMemoryOwn)
        {
            ...
            {
                listView = new BaseComponents.TableView( 0, 1 );
                listView.HeightResizePolicy = ResizePolicyType.FitToChildren;
                listView.SetKeyboardFocusable( true );
                // TODO : WIDTH Size and Position  of TableView
                FocusManager.Instance.PreFocusChange += OnKeyboardPreFocusChangeSignal;

                Window.Instance.Add(listView);
        ...
        internal ListShownSignalType AutofillListShownSignal()
        {
            ListShownSignalType ret = new ListShownSignalType(Interop.AutofillContainer.AutofillContainer_AutofillListShownSignal(swigCPtr), false);
            if (NDalicPINVOKE.SWIGPendingException.Pending) throw NDalicPINVOKE.SWIGPendingException.Retrieve();
            return ret;
        }
        ...
        private void OnListShown( IntPtr control, string presentationText )
        {
            ...
            // TODO : 아마 Control, 즉 TextField의 child로 TableView가 들어가야할테고
            // 이때 child로 기존 TableView가 있으면 -> 그 TableView를 가지고 와서 거기에 AddChild()를 하고
            // 그게 아니라 없으면 -> 새로 TableView를 가지고 와서 -> <List> 혹은 <Vector>에 넣고
            // TableView setting & AddCHild()를 해야할꺼임


            BaseComponents.TextLabel textLabel = new BaseComponents.TextLabel(presentationText);
            listView.InsertRow( listIndex );
            listView.AddChild( textLabel, new BaseComponents.TableView.CellPosition( listIndex, 0 ) );
            listView.SetCellAlignment(new BaseComponents.TableView.CellPosition( listIndex, 0 ), HorizontalAlignmentType.Center, VerticalAlignmentType.Center);
            
            if(listIndex == 0)
            {
                // The focus should be at the first content
                FocusManager.Instance.SetCurrentFocusView( textLabel );
            }

            // TODO : Focus clicked -> return that textLabel and the text?
        ...
        private BaseComponents.View OnKeyboardPreFocusChangeSignal(object source, FocusManager.PreFocusChangeEventArgs e)
        {
            if( !e.CurrentView && !e.ProposedView )
            {
                e.ProposedView = listView.GetChildAt( new BaseComponents.TableView.CellPosition( 0, 0 ) );
            }
